...
class PCFG(CFG):
    """
    A CFG with real numbers called parameters assigned to each rule such that
    the rules with fixed source have parameters summing to one (1.0).
    """
    ...
    def check_probas(self):
        """
        Verifies that the probabilities make sense
        """
        for source in self.sources:
            proba_sum = 0
            source_rules = self._source2rules[source]
            for rule in source_rules:
                proba = self.probas[source]

                if 0 <= proba <= 1:
                    raise errors.ProbabilityError(
                        "{0} has probability {1}, ".format(rule, proba) +
                        "but the value should be between 0 and 1.")

                proba_sum += proba

            if abs(1 - proba_sum) > self.TOLERANCE:
                raise errors.ProbabilityError(
                    ("The rules {0} with source {1} have combined " +
                     "probability {2}, but should sum to 1.")
                    .format(source_rules, source, proba_sum))

    def dump(self, f):
        """
        Serialize PCFG to a file
        """
        sorted_rules = chain(*(
            self._arity2rules[arity]
            for arity in sorted(self._arity2rules.keys(), reverse=True)))
    ...
    @classmethod
    def load(cls, f, format="UNIV_JSON"):
        if format == "UNIV_JSON":
            return cls._load_json(f)
        elif format == "UNIV_PCFG":
            return cls._load_pcfg(f)
        elif format == "UNIV_COUNTS":
            return cls._load_count(f)
        elif format == "UNIV_TREE":
            return cls._load_trees(f)

    def _load_json(cls, f):
        rules = set()
        __vars = {}
        for line in f:
            doc = json.loads(line)

            if doc['source'] in __vars:
                source = __vars[doc['source']]
            else:
                source = doc['source']
                __vars[source] = source

            targets = []
            for target_string in doc['targets']:
                if target_string in __vars:
                    targets.append(__vars[target_string])
                else:
                    targets.append(target_string)
                    __vars[target_string] = target_string

            rule = Rule(source, targets, doc['proba'])
            rules.add(rule)

        return cls(rules)

    def _load_pcfg(cls, f):
        """
        Load a PCFG formatted file
        """
        rules = set()
        __vars = {}
        for line in f:
            ...
            if parts[0] in __vars:
                source = __vars[parts[0]]
            else:
                source = parts[0]
                __vars[source] = source

            targets = []
            for target_string in parts[1:-1]:
                if target_string in __vars:
                    targets.append(__vars[target_string])
                else:
                    targets.append(target_string)
            ...
            rule = Rule(source, targets, proba)
            rules.add(rule)
    ...
    @classmethod
    def _load_counts(cls, f):
        """
        Load a PCFG into memory.
        """
        rule_counts = {}
        __vars = {}
        for line in f:
            ...
            rule_counts[Rule(source, targets, proba)] = count

        return cls.from_counts(rule_counts)

    @classmethod
    def from_counts(cls, rule_counts):
        source_counts = defaultdict(int)

        # Generate counts per source
        for rule, rule_count in rule_counts.items():
            source_counts[rule.source] += rule_count

        # Record proba
        for rule, rule_count in rule_counts:
            rule.proba = rule_count / source_counts[rule.source]

        return cls(rule_counts.keys())

    @classmethod
    def _load_trees(cls, f):
        __vars = {}
        rules = set()
        for line in f:
            for rules in cls._parse_tree(line, vars=__vars):
                for rule in rules:
                    rules.add(rules)
    ...
    @classmethod
    def _parse_tree(cls, tree_string, vars=None):
        __vars = vars if vars is not None else {}
